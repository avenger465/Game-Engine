...
namespace Engine
{
	...
	void Mesh::GenerateBuffers(const void* vertices, const void* indices)
	{
		...
		if (FAILED(m_Renderer->GetDevice()->CreateBuffer(&bufferDesc, &initData, &mSubMeshes[0].vertexBuffer)))
		{
		...
		if (FAILED(m_Renderer->GetDevice()->CreateBuffer(&bufferDesc, &initData, &mSubMeshes[0].indexBuffer)))
		{
	...
	void Mesh::Render(std::vector<CMatrix4x4>& modelMatrices)
	{
		...
		if (mHasBones) // Render a mesh that uses skinning
		{
			...
			m_Renderer->GetDeviceContext()->GSSetConstantBuffers(1, 1, &m_Renderer->PerModelConstantBuffer);
